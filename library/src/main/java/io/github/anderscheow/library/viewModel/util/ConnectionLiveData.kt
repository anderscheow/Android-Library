...
data class Connection(val networkInfo: NetworkInfo?, val isConnected: Boolean)

class ConnectionLiveData(val context: Context) : LiveData<Connection>() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            connectivityManagerCallback = object : ConnectivityManager.NetworkCallback() {
                override fun onAvailable(network: Network?) {
                    postValue(Connection(connectivityManager.activeNetworkInfo, true))
                }

                override fun onLost(network: Network?) {
                    postValue(Connection(connectivityManager.activeNetworkInfo, false))
                }
            }
